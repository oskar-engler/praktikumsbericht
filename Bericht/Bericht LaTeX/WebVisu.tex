...
%Die Daten die im SQL liegen werden mittels Diagrammen visualisiert, es gibt mehrere Diagrammen-Bibliotheken, die man benutzen kann,  es war wichtig, dass die Open-source, mit SQL arbeiten kann - dynamisch, viele Optionen wie Export, mehrere Linien anzeigen, mehrere Typen von Charts, Datum-anzeigen}
\pagebreak
...
%Über PHP kann man die SQL zugreifen und somit die Daten in die Diagramme einladen. Erstens muss man sich mit Datenbank verbinden. Das erfolgt mittels \textbf{connect} und in unseren Fall war das \textbf{pg_connect}, weil wir die postgreSQL benutzen. Nach der Verbindung mit SQL kann man mit den Befehlen \textbf{pg_query} die Daten aus der Datenbank auf eine Variable aufladen. Die Daten werden in eine Form gespeichert, wenn wir die Daten in den Form JSON oder CSV brauchen, müssen wir noch zu den \textbf{pg_query} die Befehlen für das \textbf{encode} hinzuschreiben. Bei der postgreSQL gibt es einen \textbf{encode} für JSON, es heißt: \textbf{json_encode}. Für die CSV gibt es keinen \textbf{encode}, so habe ich eine Funktion mit \textit{umschreiben} in die CSV Form geschrieben.

%Kollegen haben einen Server erstellt, wo ich die Website veröffentlichen soll, damit die online verfügbar ist
