...
\large{Damit die jeweiligen Listenelementen nach dem Klick immer noch auf eine Seite geöffnet werden, wurde ein 
...
Skript entwickelt. Das Ziel war, dass beim Anklicken das untergeordnete Listenelement bzw. Tabelle nach unten rutschen wird. Die jQuery anbietet solche Funktion: \textbf{slideToggle}, welche der Absicht entspricht. Man muss definieren, welche Elemente diese Funktion vornehmen sollen (Siehe Abbildung \ref{jQuery_america_slide}). Deswegen müssen die HTML Elementen mit IDs oder Klassen verbunden sein. 
...
\section{PHP}
\large{Der nächste Schwerpunkt war die Umsetzung in PHP. Es war gebraucht, dass die Website dynamisch ist. Die bereits gemachte HMTL Seite wurden mit PHP umgesetzt. Nach der Bearbeitung wird es als \textbf{.php} Datei gespeichert und auf den Server hingelegt. Der Server hat einen FTP-Zugang. Über den kann man die Dateien hochladen und dann die Seite über eine IP-Adresse besuchen. Die PHP Skript-Sprache ist eine serverseitige Sprache, das heißt, dass der Code auf den Server verarbeitet wird. In der PHP-Interpreter wird das Code verarbeitet und erzeugt eine Datei, welche an den Client zurückgegeben wird.\\
